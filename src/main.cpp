...
#if defined(NDEBUG)
# error "meta1coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf1dd69f4caba7aab989aaa83f74b5828420aa81ef745d4eae611ec0591dc162c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // meta1coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "meta1coin Signed Message:\n";
...
{
    // meta1coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // meta1coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // meta1coin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // meta1coin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // meta1coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // meta1coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf1dd69f4caba7aab989aaa83f74b5828420aa81ef745d4eae611ec0591dc162c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 01/Nov/2018 5 days left, Here’s the state of the race.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("000184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541091087;
        ...
        assert(block.hashMerkleRoot == uint256("0x77c9a922e9d2e1afbbfb452b7e5e139da1e23801ced37a8ad666594dc91b215c"));		
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb2, 0xda }; // meta1coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("meta1coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("meta1coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static meta1coinMiner(CWallet *pwallet)
{
    printf("meta1coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("meta1coin-miner");
    ...
    try { loop {
        ...
        printf("Running meta1coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("meta1coinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&meta1coinMiner, pwallet));
}
